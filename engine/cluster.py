...
class BranchAndBoundCluster(Cluster):
    ...
    def cluster(self, dgraph):
        ...
        for i in range(len(sorted_nodes_by_edge_weight)):
            sorted_nodes_by_edge_weight[i] = edge_weight_of_node.index(sorted_nodes_by_edge_weight[
                                                                           i])  # should give a list of node indexes sorted by the total weight of the node's edges
        ...
        while (len(bnbtree.live_nodes) > 0):
            ...
            bnbtree.add_node(BnBNode(1, Calc_Upper_Bound([1] + current_live_node.graph_node_list), Lower_Bound(
                [1] + current_live_node.graph_node_list)))  # if the node is added to the subgraph (x_i=1)
            bnbtree.add_node(BnBNode(1, Calc_Upper_Bound([0] + current_live_node.graph_node_list), Lower_Bound(
                [0] + current_live_node.graph_node_list)))  # if the node is not added to the subgraph (x_i=0)
